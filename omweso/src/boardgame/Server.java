...
public class Server implements Runnable {
    ...
    protected static final int DEFAULT_TIMEOUT_CUSHION = 4000;

    protected static final int FIRST_MOVE_TIMEOUT = 60000;
    protected static final int FIRST_MOVE_TIMEOUT_CUSHION = 10000;

    protected static final String DEFAULT_BOARDCLASS = "omweso.CCBoard";
    ...
    private static void printUsage() {
        System.err.println(
                ...
                "        '-t n' sets timeout. (default="+DEFAULT_TIMEOUT+")\n" +
                "        '-ft n' sets timeout for the first move. (default="+FIRST_MOVE_TIMEOUT+")\n" +
    ...
    private synchronized void processMessage(String inputLine, ClientHandler h ) {
        ...
        if( gameStarted && h.getPlayerID() != board.getTurn() ) {
            System.err.println(
               "It is currently: " + board.getNameForID(board.getTurn()) + "'s turn. " +
               "Ignoring out of turn message from " +
        ...
        // Otherwise, expect a move
            ...
            try {
                ...
                System.err.println( "Ignoring unparseable move from " +
                        h.getName() + ": " + inputLine );
                e.printStackTrace();
                return;
            }
    ...
    private synchronized void killTimeOut(int player_id) {
        ...
        endGame( "TIMEOUT " + board.getNameForID(player_id) );
    }
    ...
    private void endGame( String reason ) {
        ...
        if( logOut != null ) {
            ...
            try {
                ...
                out.print( Integer.toString(board.getTurnsPlayed()) + delim );
        ...
        public void run() {
            ...
            try {
                while (true) {
                    ...
                    if(moveIsSet()){
                        System.out.println(
                            "Player " + colour + " timeout - Ignoring move from player: " + inputLine);

                        // Move did not come in time. Using a random move instead.
                        // Random move is set by the timeOut method of the Server.
                        inputLine = getMove();
                        System.out.println(
                            "Player " + colour + " timeout - Playing random move: " + inputLine);
